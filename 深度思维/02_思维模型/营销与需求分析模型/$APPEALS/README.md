...
`$APPEALS`方法是 IBM 在 IPD 总结和分析出来的客户需求分析的一种方法。它从 8 个方面对产品进行客户需求定义和产品定位。$-产品价格（Price）；A-可获得性（Availability）；P-包装（Packaging）；P-性能（Performance）；E-易用性（Easy to use）；A-保证程度（Assurances）；L-生命周期成本（Life cycle of cost）；S-社会接受程度（Social acceptance）。

使用客户`$APPEALS`框架来确定客户的欲望与需要，建立针对每一个细分市场的产品包对应图。客户`$APPEALS` 框架的目的主要包括处理目标细分市场的全部客户欲望与需要建立客户驱动的需求集，作为投资的重点确定要想在所选细分市场获得成功必须达到的主要分界标准确定促使客户选择公司产品的主要差异。
